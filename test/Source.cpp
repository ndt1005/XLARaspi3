...
int main(int argc, char* argv[])
{
	...
	int i, j, k, t1min = 32, t1max = 69, t2min = 64, t2max = 175, t3min = 147, t3max = 256; // other variables used
	//init rmax
	for (int v = 0; v < 3; v++)
		rmax[v] = 0;
	...
	/*if (temp)
	{
		...
		t1min = (int)CV_MAT_ELEM(*threshold_matrix, int, 0, 0); t2min = (int)CV_MAT_ELEM(*threshold_matrix, int, 0, 1); t3min = (int)CV_MAT_ELEM(*threshold_matrix, int, 0, 2);
		t1max = (int)CV_MAT_ELEM(*threshold_matrix, int, 1, 0); t2max = (int)CV_MAT_ELEM(*threshold_matrix, int, 1, 1); t3max = (int)CV_MAT_ELEM(*threshold_matrix, int, 1, 2);
	}*/
	...
	cvCreateTrackbar(TrackbarName1, "F1", &t1min, 180, NULL);
	cvCreateTrackbar(TrackbarName2, "F1", &t1max, 180, NULL);

	cvCreateTrackbar(TrackbarName3, "F2", &t2min, 255, NULL);
	cvCreateTrackbar(TrackbarName4, "F2", &t2max, 255, NULL);

	cvCreateTrackbar(TrackbarName5, "F3", &t3min, 255, NULL);
	cvCreateTrackbar(TrackbarName6, "F3", &t3max, 255, NULL);
	...
	while (1)
	{
		...
		printf("xm=%d, ym=%d, rm=%d\n", rmax[0], rmax[1], rmax[2]);
		rmax[2] = 0;//khoi tao lai rmax cho vong moi
